Feature: Generator invariants (property-based)
  ...
  Scenario: Any valid Go file is a valid Go+ file
    Then for any plain Go file, parsing as Go+ succeeds with no generic methods
  ...
  Scenario: Generation is deterministic and idempotent
    Then for any Go+ package, generating twice produces identical bytes and no rewrites

  Scenario: Lowered names are independent of declaration order
    Then for any Go+ package, permuting declarations preserves the lowered function names
