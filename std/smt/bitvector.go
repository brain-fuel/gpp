...
type compactBitVectorProblem struct {
	...
	fmaCount          int
	fmas              [4]FloatingPointFMARelation
	sqrtCount         int
	sqrts             [4]FloatingPointSqrtRelation
	remCount          int
...
func (problem *compactBitVectorProblem) add(term Term[BoolSort], negated bool) bool {
	...
	case FloatingPointRoundToIntegralRelation:
		if problem.roundCount == len(problem.rounds) {
			return false
		}
		value.Negated = value.Negated != negated
	...
	case FloatingPointToBitVectorRelation:
		if problem.fpConversionCount == len(problem.fpConversions) {
			return false
		}
		value.Negated = value.Negated != negated
	...
	case FloatingPointFormatConversionRelation:
		if problem.formatCount == len(problem.formats) {
			return false
		}
		value.Negated = value.Negated != negated
	...
	case FloatingPointAddRelation:
		...
		problem.addCount++
		return true
	case FloatingPointSubRelation:
		if problem.subCount == len(problem.subs) {
			return false
		}
		value.Negated = value.Negated != negated
	...
	case FloatingPointDivRelation:
		...
		problem.divCount++
		return true
	case FloatingPointFMARelation:
		if problem.fmaCount == len(problem.fmas) {
			return false
		}
		value.Negated = value.Negated != negated
		problem.fmas[problem.fmaCount] = value
		problem.fmaCount++
		return true
	case FloatingPointSqrtRelation:
		if problem.sqrtCount == len(problem.sqrts) {
			return false
		}
		value.Negated = value.Negated != negated
		problem.sqrts[problem.sqrtCount] = value
		problem.sqrtCount++
		return true
	case FloatingPointRemRelation:
		if problem.remCount == len(problem.rems) {
			return false
		}
		value.Negated = value.Negated != negated
		problem.rems[problem.remCount] = value
		problem.remCount++
		return true
...
func solveCompactBitVectorAssertions(assertions []Term[BoolSort]) (checkOutcome, bool) {
	...
	for _, relation := range problem.formats[:problem.formatCount] {
		...
		sourceWidth := relation.SourceExponentBits +
			relation.SourceSignificandBits
		if !found || assigned.Width() != sourceWidth {
			return checkOutcome{}, false
		}
		converted := floatingPointConvertFormat(
			relation.Mode,
			relation.TargetExponentBits, relation.TargetSignificandBits,
			FloatingPointFromBits(
				relation.SourceExponentBits,
				relation.SourceSignificandBits,
				assigned,
	...
	for _, relation := range problem.fmas[:problem.fmaCount] {
		...
		fused := floatingPointFMA(
			...
			FloatingPointFromBits(
				relation.ExponentBits, relation.SignificandBits, addend,
			),
		)
		holds := EqualBitVectorValue(FloatingPointBits(fused), relation.Value)
		if holds == relation.Negated {
			return checkOutcome{status: checkUnsat}, true
		}
	}
	for _, relation := range problem.sqrts[:problem.sqrtCount] {
		...
		holds := EqualBitVectorValue(FloatingPointBits(root), relation.Value)
		if holds == relation.Negated {
			return checkOutcome{status: checkUnsat}, true
		}
	}
	for _, relation := range problem.rems[:problem.remCount] {
		...
		holds := EqualBitVectorValue(FloatingPointBits(remainder), relation.Value)
		if holds == relation.Negated {
			return checkOutcome{status: checkUnsat}, true
		}
	}
...
func evaluateBitVector(term any, model bitVectorModel, integers integerModel) (BitVectorValue, bool) {
	switch value := term.(type) {
	case floatingPointRoundToIntegralBitVector:
		...
		rounded := floatingPointRoundToIntegral(
			value.mode,
			FloatingPointFromBits(
				value.exponentBits, value.significandBits, bits,
			),
		)
		return FloatingPointBits(rounded), true
	case floatingPointToBitVectorTermValue:
		bits, ok := evaluateBitVector(value.value, model, integers)
		if !ok || bits.Width() != value.exponentBits+value.significandBits {
			return BitVectorValue{}, false
		}
		converted, valid := floatingPointToBitVector(
			value.mode,
		...
		if !valid {
			// SMT-LIB leaves invalid conversion results unspecified. Zero is
			// our stable model choice.
	...
	case floatingPointFormatConversionTermValue:
		bits, ok := evaluateBitVector(value.value, model, integers)
		sourceWidth := value.sourceExponentBits + value.sourceSignificandBits
		if !ok || bits.Width() != sourceWidth {
			return BitVectorValue{}, false
		}
		converted := floatingPointConvertFormat(
			value.mode,
			value.targetExponentBits, value.targetSignificandBits,
			FloatingPointFromBits(
				value.sourceExponentBits, value.sourceSignificandBits, bits,
			),
		)
		return FloatingPointBits(converted), true
	case If[BitVecSort]:
