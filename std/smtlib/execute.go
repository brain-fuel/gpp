...
type dynamicTerm struct {
	...
	floatingPointRem        *dynamicFloatingPointRem
...
type dynamicFloatingPointFromReal struct {
	exponentBits, significandBits int
	symbolID                      int
	mode                          smt.FloatingPointRoundingMode
}
...
type dynamicFloatingPointDiv struct {
	exponentBits, significandBits int
	leftSymbolID, rightSymbolID   int
	mode                          smt.FloatingPointRoundingMode
}
...
type dynamicFloatingPointRem struct {
	exponentBits, significandBits int
	leftSymbolID, rightSymbolID   int
}
...
func buildApplication(operator string, terms []dynamicTerm) (dynamicTerm, error) {
	...
	if operator == "fp.to_ieee_bv" && len(terms) == 1 &&
		...
		return dynamicTerm{
			...
			floatingPointFromReal: terms[0].floatingPointFromReal,
			...
			floatingPointRem:      terms[0].floatingPointRem,
	...
	if operator == "fp.div" && len(terms) == 3 &&
		terms[0].sort == sortRoundingMode &&
		terms[1].sort == sortFloatingPoint &&
		terms[2].sort == sortFloatingPoint &&
		terms[1].exponentBits == terms[2].exponentBits &&
		...
		if left.bitVectorExact && right.bitVectorExact {
			...
			bits := smt.FloatingPointBits(quotient)
			return dynamicTerm{
				sort: sortFloatingPoint, bitWidth: left.bitWidth,
				exponentBits: left.exponentBits, significandBits: left.significandBits,
				bitVector:      smt.BitVectorTerm(bits),
		...
		if left.floatingPointSymbol == 0 || right.floatingPointSymbol == 0 {
			return dynamicTerm{}, fmt.Errorf("fp.div currently requires ground values or direct symbols")
		}
		return dynamicTerm{
			sort: sortFloatingPoint, bitWidth: left.bitWidth,
			exponentBits: left.exponentBits, significandBits: left.significandBits,
	...
	if operator == "fp.rem" && len(terms) == 2 &&
		terms[0].sort == sortFloatingPoint &&
		terms[1].sort == sortFloatingPoint &&
		...
		if left.bitVectorExact && right.bitVectorExact {
			remainder := smt.FloatingPointRem(
				smt.FloatingPointFromBits(
					left.exponentBits, left.significandBits, left.bitVectorValue,
				),
				smt.FloatingPointFromBits(
			...
			bits := smt.FloatingPointBits(remainder)
			return dynamicTerm{
				sort: sortFloatingPoint, bitWidth: left.bitWidth,
				exponentBits: left.exponentBits, significandBits: left.significandBits,
				bitVector:      smt.BitVectorTerm(bits),
		...
		if left.floatingPointSymbol == 0 || right.floatingPointSymbol == 0 {
			return dynamicTerm{}, fmt.Errorf("fp.rem currently requires ground values or direct symbols")
		}
		return dynamicTerm{
			sort: sortFloatingPoint, bitWidth: left.bitWidth,
			exponentBits: left.exponentBits, significandBits: left.significandBits,
	...
	case "=":
		if len(terms) == 2 {
			...
			converted, exact := terms[0], terms[1]
			if converted.floatingPointConversion == nil {
				converted, exact = terms[1], terms[0]
			}
			if converted.floatingPointConversion != nil &&
				exact.bitVectorExact &&
				converted.bitWidth == exact.bitWidth &&
				converted.floatingPointConversion.symbolID != 0 {
				relation := converted.floatingPointConversion
				return dynamicTerm{
					sort: sortBool,
					boolean: smt.NewFloatingPointToBitVectorRelation(
						relation.exponentBits, relation.significandBits,
			...
			fromReal, exact := terms[0], terms[1]
			if fromReal.floatingPointFromReal == nil {
				fromReal, exact = terms[1], terms[0]
			}
			if fromReal.floatingPointFromReal != nil &&
				exact.bitVectorExact &&
				fromReal.bitWidth == exact.bitWidth &&
				fromReal.floatingPointFromReal.symbolID != 0 {
				relation := fromReal.floatingPointFromReal
				return dynamicTerm{
					sort: sortBool,
					boolean: smt.NewFloatingPointFromRealRelation(
						relation.exponentBits, relation.significandBits,
						relation.symbolID, relation.mode,
						exact.bitVectorValue,
					),
			...
			quotient, exact := terms[0], terms[1]
			if quotient.floatingPointDiv == nil {
				quotient, exact = terms[1], terms[0]
			}
			if quotient.floatingPointDiv != nil && exact.bitVectorExact &&
				quotient.bitWidth == exact.bitWidth {
				relation := quotient.floatingPointDiv
				return dynamicTerm{
					sort: sortBool,
					boolean: smt.NewFloatingPointDivRelation(
						relation.exponentBits, relation.significandBits,
						relation.leftSymbolID, relation.rightSymbolID,
						relation.mode, exact.bitVectorValue,
					),
			...
			remainder, exact := terms[0], terms[1]
			if remainder.floatingPointRem == nil {
				remainder, exact = terms[1], terms[0]
			}
			if remainder.floatingPointRem != nil && exact.bitVectorExact &&
				remainder.bitWidth == exact.bitWidth {
				relation := remainder.floatingPointRem
				return dynamicTerm{
					sort: sortBool,
					boolean: smt.NewFloatingPointRemRelation(
						relation.exponentBits, relation.significandBits,
						relation.leftSymbolID, relation.rightSymbolID,
						exact.bitVectorValue,
					),
...
func buildIndexedBitVectorApplication(operator string, parameters []int, terms []dynamicTerm) (dynamicTerm, error) {
	if operator == "fp.to_ubv" || operator == "fp.to_sbv" {
		...
		width, value := parameters[0], terms[1]
		signed := operator == "fp.to_sbv"
		if value.bitVectorExact {
			floating := smt.FloatingPointFromBits(
				value.exponentBits, value.significandBits,
				value.bitVectorValue,
			)
			var converted smt.BitVectorValue
			if signed {
				converted, _ = smt.FloatingPointToSignedBitVector(
					width, terms[0].roundingMode, floating,
				)
			} else {
				converted, _ = smt.FloatingPointToUnsignedBitVector(
					width, terms[0].roundingMode, floating,
				)
			}
			return dynamicTerm{
				sort: sortBitVector, bitWidth: width,
				bitVector:      smt.BitVectorTerm(converted),
				bitVectorExact: true, bitVectorValue: converted,
			}, nil
		}
		var converted smt.Term[smt.BitVecSort]
		if signed {
			converted = smt.FloatingPointToSignedBitVectorTerm(
				value.exponentBits, value.significandBits, width,
				value.bitVector, terms[0].roundingMode,
			)
		} else {
			converted = smt.FloatingPointToUnsignedBitVectorTerm(
				value.exponentBits, value.significandBits, width,
				value.bitVector, terms[0].roundingMode,
			)
		}
		return dynamicTerm{
			sort: sortBitVector, bitWidth: width, bitVector: converted,
			floatingPointConversion: &dynamicFloatingPointConversion{
				exponentBits:    value.exponentBits,
				significandBits: value.significandBits,
				width:           width, symbolID: value.floatingPointSymbol,
				mode: terms[0].roundingMode, signed: signed,
			},
		}, nil
	}
	if operator == "to_fp" {
		if len(parameters) == 2 && parameters[0] >= 2 && parameters[1] >= 2 &&
			len(terms) == 2 && terms[0].sort == sortRoundingMode &&
			(terms[1].sort == sortReal ||
				terms[1].sort == sortNumber && terms[1].real != nil) {
			return buildFloatingPointFromRealApplication(
...
func buildFloatingPointFromRealApplication(
	exponentBits, significandBits int,
	mode, value dynamicTerm,
) (dynamicTerm, error) {
	if rational, exact := smt.ExactRealConstant(value.real); exact {
		converted := smt.FloatingPointFromRational(
			exponentBits, significandBits, mode.roundingMode, rational,
		)
	...
	symbol, direct := value.real.(smt.RealSymbol)
	if !direct {
		return dynamicTerm{}, fmt.Errorf(
			"real to floating-point conversion currently requires an exact value or direct symbol",
		)
	}
	return dynamicTerm{
		sort:         sortFloatingPoint,
		...
		floatingPointFromReal: &dynamicFloatingPointFromReal{
			exponentBits: exponentBits, significandBits: significandBits,
			symbolID: symbol.ID, mode: mode.roundingMode,
		},
	}, nil
}
