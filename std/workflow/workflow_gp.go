...
//
//goplus:dep NextStandardStart(_ Saga, value schedule.Schedule[5], after time.Time) schedule.NextResult
func NextStandardStart(_ Saga, value schedule.Schedule, after time.Time) schedule.NextResult {
	return schedule.NextStandard(value, after)
}

//goplus:dep NextSecondsStart(_ Saga, value schedule.Schedule[6], after time.Time) schedule.NextResult
func NextSecondsStart(_ Saga, value schedule.Schedule, after time.Time) schedule.NextResult {
	return schedule.NextSeconds(value, after)
}

func Run(ctx context.Context, journal Journal, saga Saga) error {
	if saga.ID == "" || saga.Kind == "" {
		return fmt.Errorf("workflow: ID and Kind are required")
	}
	record, found, err := journal.Load(ctx, saga.ID)
	if err != nil {
		return err
	}
	if found && record.Kind != saga.Kind {
		return fmt.Errorf("workflow %q is %q, not %q", saga.ID, record.Kind, saga.Kind)
	}
	if !found {
	...
	for record.Next < len(saga.Steps) {
		if err := saga.Steps[record.Next].Run(ctx); err != nil {
			return fmt.Errorf("workflow %s step %s: %w", saga.ID, saga.Steps[record.Next].Name, err)
		}
		record.Next++
		if err := journal.Save(ctx, record); err != nil {
			return err
	...
	for i := record.Next - 1; i >= 0; i-- {
		if saga.Steps[i].Compensate != nil {
			if err := saga.Steps[i].Compensate(ctx); err != nil {
				return fmt.Errorf("workflow %s compensate %s: %w", saga.ID, saga.Steps[i].Name, err)
			}
		}
	}
