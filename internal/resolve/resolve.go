// Package resolve rewrites Go+ method-syntax uses of generic methods into
...
func Fixpoint(in *Input) (*Output, error) {
	...
	for iter := 0; ; iter++ {
		...
		diags = passDiags
		if editCount == 0 {
			// Erasure can orphan imports whose only uses were index
			// vocabulary (states.Open in a dropped instantiation). Blank
...
func buildRegistry(reg *registry.Registry, roots []*packages.Package, in *Input) error {
	...
	packages.Visit(roots, nil, func(pkg *packages.Package) {
		...
		// reconstruction is PACKAGE-WIDE — an indexed enum may reference a
		// domain or another enum in a sibling file — so files gather
		// first; the other marker families stay per-file (their
		// references resolve through the registry by qualified key).
		...
		for _, file := range pkg.GoFiles {
			...
			if v := emit.Vintage(src); v != "" && version.Newer(v, version.Version) {
				record(fmt.Errorf("%s was generated by goplus %s; this toolchain is %s — upgrade with: go install goforge.dev/goplus/cmd/goplus@%s",
					file, v, version.Version, v))
				continue
			}
			pkgFiles[file] = src
		}
		extern := func(importPath, name string) (*registry.Enum, bool) {
			return reg.LookupEnum(importPath, name)
		}
		if enums, err := registry.EnumsFromPackageMarkers(pkg.PkgPath, pkgFiles, extern); err == nil {
			for _, e := range enums {
