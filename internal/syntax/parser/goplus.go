...
// parseTypeSpecType parses the Type of a type spec, recognizing the Go+
...
// goplusExtOp reports whether the current token starts a claimed `|>`,
// `>>>`, or `>=>` sequence (adjacency-checked).
func (p *parser) goplusExtOp() bool {
...
// parseFileExt mirrors ParseFile but also returns the file's Go+
